...
local function atm_enter()
	...
	_source = source
	if user_id ~= nil then
		vRP.getAtmCode(user_id, function(code)
			vRPclient.atmEnter(_source,{vRP.getMoney(user_id),vRP.getBankMoney(user_id),code})
		end)
	end
end

local function bank_enter()
	local user_id = vRP.getUserId(source)
	_source = source
	if user_id ~= nil then
		vRP.getUserIdentity(user_id, function(identity)
			vRPclient.bankEnter(_source,{identity,vRP.getMoney(user_id),vRP.getBankMoney(user_id)})
		end)
	end
end

local function atm_leave()
	vRPclient.atmLeave(source,{})
end

function tvRP.build_client_atms(source)
	local user_id = vRP.getUserId(source)
	if user_id ~= nil then
		for k,v in pairs(atms) do
			local x,y,z,bank = table.unpack(v)
			
			vRPclient.addBlip(source,{x,y,z,272,4,lang.atm.title()})
			vRPclient.addMarker(source,{x,y,z,0.7,0.7,0.6,0,255,125,125,150})
			if bank then
				vRP.setArea(source,"vRP:atm"..k,x,y,z,1,1.5,bank_enter,atm_leave)
			else
